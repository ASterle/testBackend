...
public class StorageIT {
    ...
    public void createFile_binary() throws IOException, URISyntaxException {
        ...
        final String fileContent = Base64.getEncoder().encodeToString(Files.readAllBytes(file.toPath()));

        final List<File> storageFile = new ArrayList<>(Collections.singleton(new File("imageFile", fileContent)));
    ...
    public void createFile_binaries() throws IOException, URISyntaxException {
        ...
        final String fileContent = Base64.getEncoder().encodeToString(Files.readAllBytes(file.toPath()));
        final String fileContent2 = Base64.getEncoder().encodeToString(Files.readAllBytes(file2.toPath()));
